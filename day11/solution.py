...
empty_spaces_x = get_empty_space(universe)
empty_spaces_y = get_empty_space(["".join(s) for s in zip(*universe)])
...
for idx, p1 in enumerate(galaxies):
    for p2 in galaxies[idx+1:]:
        ...
        scale_factor = get_scale_factor(empty_spaces_x, min_x, max_x) + get_scale_factor(empty_spaces_y, min_y, max_y)
